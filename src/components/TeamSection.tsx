...
export default function TeamSection() {
    ...
    const MemberCard = ({ member, index, baseDelay = 0 }: { member: typeof team[0], index: number, baseDelay?: number }) => (
        <motion.div
            key={member.id}
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: baseDelay + (index * 0.2) }}
                ...
                <div className="flex flex-col gap-12 md:gap-20">
                    ...
                    <div className="flex flex-wrap justify-center gap-12 md:gap-20">
                        {row2.map((member, index) => (
                            <MemberCard key={member.id} member={member} index={index} baseDelay={0.4} />
                        ))}
                    </div>
                </div>
            </div>
